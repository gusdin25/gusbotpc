...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '6281997826183@c.us'
        ...
        function cekumur(obj){
            if (obj === true){
                ...
                return tobz.reply(from, `Kamu belum cukup umur untuk menggunakan Gus, min 16 tahun\n\nKamu bisa mendaftar ulang dengan cara donasi terlebih dahulu, bales ${prefix}donasi\nHubungi Owner : wa.me/6281997826183`, id) //if user is not registered
            }
        }
        ...
        // FUNCTION
                ...
                for (let i of stickerspam){
                    if(i.id === id){
                        if (i.msg >= 12) {
                            found === true 
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu telah SPAM STICKER di grup, kamu akan di kick otomatis oleh GusBot`, id).then(() => {
                                tobz.removeParticipant(groupId, id)
                            })
                            return true;
                        }else if(i.msg >= 12){
                            found === true
                            tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗦𝗣𝗔𝗠 𝗦𝗧𝗜𝗖𝗞𝗘𝗥 」*\nKamu terdeteksi spam sticker!\nMohon tidak spam 5 sticker lagi atau nomor akan di kick oleh GusBot!`, id)
                            return true
                        }else{
                            found === true
                            return false;
            ...
            for (let i of msgBadword){
                if(i.id === id){
                    ...
                    if (msg >= 3) { // 3X BADWORD AKAN TERKENA KICK
                        kasar === true 
                        tobz.reply(from, `*「 𝗔𝗡𝗧𝗜 𝗕𝗔𝗗𝗪𝗢𝗥𝗗 」*\nKamu telah berkata kasar di grup ini, kamu akan di kick otomatis oleh GusBot!`, id).then(() => {
                            tobz.removeParticipant(groupId, id)
                        })
                        return true;
                    }else{
	...
	if (isGroupMsg && AntiStickerSpam && !isGroupAdmins && !isAdmin && !isOwner){
        ...
        if(!isCmd && isKasar && isGroupMsg && isBadword && !isGroupAdmins) { 
            ...
            if(isBadwordMsg(serial)) return
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin GusBot!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner GusBot!', id)
        ...
        case prefix+'banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner GusBot!', id)
        ...
        case prefix+'ttp2':
            ...
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#ttp2 [ Teks ]*, contoh *#ttp2 GusBot*`, id)
        ...
        case prefix+'magernulis1': // BY MFARELS
            ...
            spawn('convert', [
                ...
                '+1010+1010',
                farelzahra,
                './mager/magernulis√/magernulis1hasil.jpg'
            ])
            .on('error', () => tobz.reply(from, 'Error Bjeer', id))
            .on('exit', () => {
                tobz.sendImage(from, './mager/magernulis√/magernulis1hasil.jpg', 'magernulis.jpg', '*Sukses Nulis DiBuku✓*\n\n*Instagram : @gusdin_*\n\n*© Powered By GusDin | GUS BOT*', id)
            })
            break  // BY MFARELS
        ...
        case prefix+'lovemessage':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *${prefix}lovemessage [ Teks ]*, contoh *${prefix}lovemessage GusDin*`, id)
        ...
        case prefix+'blackpink':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#blackpink [ Teks ]*, contoh *#blackpink GusDin*`, id)
            tobz.reply(from, mess.wait, id)
        ...
        case prefix+'thunder':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#thunder [ Teks ]*, contoh *#thunder GusDin*`, id)
            tobz.reply(from, mess.wait, id)
            const thndr = body.slice(9)
            if (thndr.length > 10) return tobz.reply(from, '*Teks Terlalu Panjang!*\n_Maksimal 10 huruf!_', id)
            await tobz.sendFileFromUrl(from, `https://docs-jojo.herokuapp.com/api/thunder?text=${thndr}`, 'thndr.jpg', '', id)
            break
        case prefix+'pornhub':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Gusbot|Dev GusDin*`, id)
            ...
            if (argz.length >= 2) {
                ...
                tobz.sendFileFromUrl(from, `https://mhankbarbars.herokuapp.com/api/textpro?theme=pornhub&text1=${lpornhub}&text2=${lpornhub2}`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#pornhub [ |Teks1|Teks2 ]*, contoh *#pornhub |Gusbot|Dev GusDin*`, id)
            }
            break
        case prefix+'glitch':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            if (args.length === 1) return tobz.reply(from, `Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Gusbot|Dev GusDin*`, id)
            argz = body.trim().split('|')
            if (argz.length >= 2) {
                tobz.reply(from, mess.wait, id)
                ...
                tobz.sendFileFromUrl(from, `https://tobz-api.herokuapp.com/api/textpro?theme=glitch&text1=${glitch1}&text2=${glitch2}`)
                await limitAdd(serial)
            } else {
                await tobz.reply(from, `Wrong Format!\n[❗] Kirim perintah *#glitch [ |Teks1|Teks2 ]*, contoh *#glitch |Gusbot|Dev GusDin*`, id)
            }
            break
        case prefix+'daftar':  // NAMBAHIN NOMOR DI DATABASE
                ...
                const umurnye = argz[2]
                    ...
                    if(umurnye >= 40) return await tobz.reply(from, 'Kamu terlalu tua, kembali lagi ke masa muda untuk menggunakan GusDin', id)
...
Total Pengguna yang telah terdaftar ${pendaftar.length}`)
            ...
            case prefix+'daftarulang':
                    if (!isAdmin) return tobz.reply(from, 'Command ini hanya dapat digunakan oleh admin GusBot', id)  
...
${desc}`)
        ...
        case prefix+'owner':
        case prefix+'creator':
            tobz.sendContact(chatId, `6281997826183@c.us`)
        ...
        case prefix+'resetbadword':
            ...
            if(cekumur(cekage)) return
                    ...
                    if (!args.length === 1) return tobz.reply(from, 'Masukkan nomornya, *GUNAKAN AWALAN 62*\ncontoh: #resetbadword 6281997826183 / #resetbadword @member') 
                    const nomer = args[1]
        ...
        case prefix+'simi':
            ...
            if (args[1].toLowerCase() === 'enable') {
                ...
                if(cek){
                    return tobz.reply(from, `Simsimi Sudah diaktifkan`, id)
                } else {
                simi_.push(chat.id)
                ...
                fs.writeFileSync('./lib/database/Simsimi.json', JSON.stringify(simi_))
                tobz.reply(from, 'Simsimi berhasil di nonaktifkan!', id)
                }
            } else {
                tobz.reply(from, 'Pilih enable atau disable udin!', id)
            }
        ...
        case prefix+'loli':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const loli = await axios.get(`https://api.fdci.se/rep.php?gambar=loli`)
        ...
        case prefix+'waifu':
            ...
            if(cekumur(cekage)) return
            if (isLimit(serial)) return tobz.reply(from, `Maaf ${pushname}, Kuota Limit Kamu Sudah Habis, Ketik ${prefix}limit Untuk Mengecek Kuota Limit Kamu`, id)
            const waifu = await axios.get('https://api.fdci.se/rep.php?gambar=waifu')
            tobz.sendFileFromUrl(from, loly.result, 'waifu.jpeg', '*WAIFU*', id)
            await limitAdd(serial)
            break
        case prefix+'husbu':
            if(isReg(obj)) return
...
🌐️ *URL*: ${url}`
	...
	case prefix+'bitly':
        ...
        case prefix+'nhentai': // SEARCH NHENTAI
            ...
            if(cekumur(cekage)) return
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/6281997826183', id)
          break
        ...
        case prefix+'wiki':
            ...
            const wiki = await axios.get(`https://st4rz.herokuapp.com/api/wiki?q=${queryz_}`)
            if (wiki.data.error) {
...
➸ *Total Rekaman:* ${recording}`
        ...
        case prefix+'':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isSimi) return tobz.reply(from, 'command/Perintah Simi belum di aktifkan!', id)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'nulis':
            ...
            const ngetikk = await axios.get('http://salism3.pythonanywhere.com/write?text='+ ngettik)
        ...
        case prefix+'addbadword':
            if (!isAdmin) return tobz.reply(from, `Perintah ini hanya bisa di gunakan oleh Admin GusBot!`, id)
        ...
        case prefix+'bc': // KASIH CREDIT DONG KALO COPAS
            if (!isOwner) return tobz.reply(from, `Perintah ini hanya untuk Owner GusBot`, id)
                bctxt = body.slice(4)
                txtbc = `*「 GUSBOT BROADCAST 」*\n\n${bctxt}`
        ...
        case prefix+'omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner GusBot', id)
        ...
        case prefix+'okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin GusBot', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case prefix+'oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin GusBot', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case prefix+'oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin GusBot', id)
            await tobz.sendText(from,'GUS DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case prefix+'leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case prefix+'join':
            ...
            if (check.size < minMem) return tobz.reply(from, 'Member group tidak melebihi 3, bot tidak bisa masuk', id)
            if (check.status === 200) {
        ...
        case prefix+'sider':
            if (!isGroupMsg) return tobz.reply(from, `Perintah ini hanya bisa di gunakan dalam group!`, id)                
            if (!quotedMsg) return tobz.reply(from, `Tolong Reply Pesan GusBot`, id)
            if (!quotedMsgObj.fromMe) return tobz.reply(from, `Tolong Reply Pesan GusBot`, id)
            try {
                ...
                console.log(err)
                tobz.reply(from, `Maaf, Belum Ada Yang Membaca Pesan GusBot atau Mereka Menonaktifkan Read Receipts`, id)    
            }
            break
        ...
        case prefix+'getses':
            ...
            break
        case prefix+'gusadmin':
            let admn = `This is list of GusBot Admin\nTotal : ${adminNumber.length}\n`
...
• Limit: ${limitCount-updated.limit}`
        ...
        case prefix+'addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner GusBot!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin GusBot!', id)
                }
            break
        case prefix+'deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner GusBot!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin GusBot!', id)
            break
        case prefix+'block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner GusBot!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case prefix+'ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin GusBot!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if ((adminNumber).includes(mentionedJidList[i])) return tobz.reply(from,`Maaf ${pushname}, Kamu tidak bisa banned Admin GusBot!`, id)
        ...
        // LIST MENU
        case prefix+'runtime':
            tobz.reply(from, `GusBot telah aktif selama :\n${cts}`, id)
            break
        case prefix+'menu':
        case prefix+'help':
            tobz.sendText(from, help(prefix, cts, pendaftar))
            break
        case prefix+'gusgroup':
            tobz.reply(from, `Link Group GusBot : https://chat.whatsapp.com/\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        ...
        case prefix+'adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin GusBot', id)
